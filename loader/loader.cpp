...
constexpr int wnd_height = 300;

static int selected_proc_idx = -1;
static std::vector<proc_cache_struct> proc_list_cache;

static int load_platform_mode = 1;

IsWow64Process_t imported_IsWow64Process = nullptr;
...
bool update_proc_list_cache()
{
	...
	while (Process32NextW(hnd, &proc_entry))
	{
		...
		mb_proc_name.append(" (" + std::to_string(proc_entry.th32ProcessID) + ")");

		proc_list_cache.push_back({
			mb_proc_name, proc_entry.th32ProcessID, idx++
		});
	}
...
void draw_imgui()
{
	...
	if (ImGui::IsItemHovered())
		ImGui::SetTooltip("Search for process by name");
	...
	if (ImGui::BeginListBox("", ImVec2{ static_cast<float>(viewport.Width) - 16.f, 138.f }))
	{
		...
		{
			if (search_buffer[0] != '\0' && proc.name.find(search_buffer) == std::string::npos)
				continue;

			if (ImGui::Selectable(proc.name.c_str(), selected_proc_idx == proc.index))
	...
	if (imported_IsWow64Process)
	{
		ImGui::RadioButton("Auto", &load_platform_mode, 0);
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip("Automatically determine the client\nplatform to use on load");
		ImGui::SameLine();
	}

	ImGui::RadioButton("x86", &load_platform_mode, 1);
	ImGui::SameLine();
	
	ImGui::RadioButton("x64", &load_platform_mode, 2);
	ImGui::SameLine();
...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nShowCmd)
{
	...
	imported_IsWow64Process = reinterpret_cast<IsWow64Process_t>(GetProcAddress(GetModuleHandleW(L"kernel32"), "IsWow64Process"));
